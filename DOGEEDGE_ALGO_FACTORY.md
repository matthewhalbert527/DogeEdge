...
- generator-v2 readiness artifacts: `snapshots/reject_stream_summary.json`, `snapshots/replay_parity_report.json`, and `snapshots/executable_readiness_gate.json`;
...
Review snapshots now fail closed in their own gate artifacts as well: below 80% official coverage, `settlement_coverage_report.json` marks promotion-grade scoring unavailable; below 95%, it marks any step beyond paper unavailable. The generated `official_settlements` files are durable local exports of official rows currently present in decision/trade evidence. If no official rows exist, the files are still emitted and the report records `official_settlement_rows_absent`. Coverage is also exported by family and by candidate so review can tell whether one family is blocked by missing truth while another is ready.
...
The current research-supported executable families are `sweep-scalp` and `sweep-liquidity-imbalance`, with `paper` and `paper-variant` serving as baselines. `sweep-model` remains a research family but is lab-only for unattended auto-minting until it has executable exact-linkage. Other live families remain telemetry-only and are eligible for adapter work, not trusted ranking.

Every research metric now receives a deterministic `researchCandidateId` and `candidateConfigHash` built from family, normalized params, schema/source semantics, execution/cost/risk versions, seed, run id, and config hash. Family overlap is useful only for adapter coverage; exact candidate linkage is the promotion-relevant contract. If a live or paper row lacks `researchCandidateId`, the export marks it telemetry-only/non-promotable and the scheduler budget report assigns it no normal exploitation budget.

Generated paper/live rows now persist `researchCandidateId`, `candidateConfigHash`, `sourceResearchAlgoId`, `sourceRunId`, `sourceSnapshotHash`, and `promotionVerdictAtInstall`. The bundle exports `exact_link_summary.json` and `supported_live_linkage.tsv.gz` with exact-linked, missing-link, family-only, and unsupported counts. Supported rows missing exact identity are watch-only and receive zero normal scheduler budget.

Simulator calibration is reported by regime from realized paper decisions, fills, rejects, partial-fill ratios, and slippage. The calibration policy is tightening-only: paper evidence can reduce fill assumptions or increase cost/slippage assumptions, but it cannot loosen promotion gates.
