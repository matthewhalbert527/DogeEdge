...
export function buildExecutableReadinessGate({
  ...
  const reasonCodes = [
    ...
    ...(representedDays <= 0 ? ["represented_days_missing"] : representedDays < config.minRepresentedDays ? ["represented_days_below_threshold"] : []),
    ...(independentMarkets <= 0 ? ["independent_markets_missing"] : independentMarkets < config.minIndependentMarkets ? ["independent_markets_below_threshold"] : []),
  ...
  return {
    ...
    representedDaysReady: representedDays >= config.minRepresentedDays,
    independentMarketsReady: independentMarkets >= config.minIndependentMarkets,
...
export function readinessKpisFromGate(gate = {}, { settlementCoverageReport = {}, replayParityReport = {}, exactLinkSummary = {}, evidenceProbeSummary = {} } = {}) {
  return {
    ...
    representedDays: numeric(gate.representedDays),
    representedDaysReady: gate.representedDaysReady === true,
    independentMarkets: numeric(gate.independentMarkets),
    independentMarketsReady: gate.independentMarketsReady === true,
