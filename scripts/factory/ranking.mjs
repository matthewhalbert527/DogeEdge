...
export function rankFactoryMetrics(metrics, options = {}) {
  ...
  const withStats = metrics.map((metric) => {
    ...
    const dsr = deflatedSharpeRatio(metric, multipleTestingPenalty);
    const pbo = pboApproximation(metric);
    const concentration = concentrationMetrics(metric);
    const adjustedConfidence = roundRatio(Math.max(0, Math.min(1, dsr * (1 - pbo) - multipleTestingPenalty)));
    const robustScore = robustScoreFor(metric, { adjustedConfidence, concentration, multipleTestingPenalty });
...
function robustScoreFor(metric, context) {
  ...
  return roundRatio(pnl + sampleBonus + consistency + confidence - risk - concentrationPenalty - ciPenalty - context.multipleTestingPenalty * 20);
}
...
function deflatedSharpeRatio(metric, penalty) {
  const psr = probabilisticSharpeRatio(metric);
